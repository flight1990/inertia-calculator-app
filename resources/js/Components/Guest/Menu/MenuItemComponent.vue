    ...
    <template v-for="item in menu" :key="item.id">
        ...
        <AccordionItem v-if="item.children.length" :value="item.id">
            ...
            <div v-if="item.url" class="flex items-center justify-between gap-2">
                ...
                <AccordionTrigger
                    class="py-2.5 px-2.5 text-sm text-gray-700 rounded-lg hover:bg-gray-100 data-[state=open]:bg-gray-100 group">
                    <svg
        ...
        <div v-else-if="item.isActive"
             class="flex items-center gap-x-3.5 py-2 px-2.5 text-sm text-gray-700 rounded-lg hover:bg-gray-100 select-none">
            {{ item.name }}
        </div>
